...
crc32_tbl =[0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3, 0x0EDB8832, 0x79DCB8A4,
...
def crc_calc(cnt, crc, buf):
    tbl_idx = 0
    arry_idx = 0
    calc_crc = crc
    
    for arry_idx in range(cnt):
        tbl_idx = (calc_crc ^ buf[arry_idx]) & 0xFF
        calc_crc = (calc_crc >> 8) ^ crc32_tbl[tbl_idx]
    
    return calc_crc & 0xFFFFFFFF  # Ensure the result is a 32-bit unsigned integer
def test_crc_calc():
    # Sample data
    data = [
    ...
    # Expected CRC value
    expected_crc = 0xA6E6BF30
    
    # Initial CRC value (usually 0x00000000)
    initial_crc = 0x00000000
    
    # Calculate CRC using the crc_calc function
    calculated_crc = crc_calc(len(data), initial_crc, data)
    
    # Print the calculated CRC
    print("Calculated CRC: 0x{:X}".format(calculated_crc))
    
    # Check if the calculated CRC matches the expected CRC
    if calculated_crc == expected_crc:
        print("Test Passed: CRC Calculation is correct!")
    else:
        print("Test Failed: CRC Calculation is incorrect.")
