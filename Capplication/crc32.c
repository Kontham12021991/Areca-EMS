...
unsigned int crc32_tbl[256] ={0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3, 0x0EDB8832, 0x79DCB8A4,
...
unsigned int crc_calc(unsigned int cnt, unsigned int crc, unsigned char *buf);
...
// // write the verfify Crc32 function 
// bool verifyCrc32(uint8_t *data, size_t length) {

//     uint16_t dataLength = length-4 ;

//     printf("dataLength %d\n",dataLength);
    
//     uint16_t var_length = dataLength;

//     uint32_t calculatedCrc = 0;
//     uint32_t ExpectedCRCvalue = data[var_length+3] << 24 | data[var_length+2] << 16 | data[var_length+1] << 8 | data[var_length];
//     printf("ExpectedCRCvalue 0x%08x\n",ExpectedCRCvalue);

//     // calculate the crc of the data excluding the last 4 bytes which contain the
//     // length of the data
  
//     calculatedCrc = crc32(data, dataLength);
  

//     if (ExpectedCRCvalue == calculatedCrc)
//     {
//         printf("CRC32: 0x%08x\n", calculatedCrc); // Print CRC value to serial monitor.
//         return true;
...
bool verifyCrc32(uint8_t *data, size_t length) {
    // Error handling for length less than 4
    if (length < 4) {
        printf("Error: Data length too short to contain valid CRC.\n");
        return false;
    }
    ...
    uint32_t ExpectedCRCvalue = (uint32_t)data[dataLength + 3] << 24 |
                                (uint32_t)data[dataLength + 2] << 16 |
                                (uint32_t)data[dataLength + 1] << 8  |
                                (uint32_t)data[dataLength];

    printf("Expected CRC value: 0x%08x\n", ExpectedCRCvalue);

    // Calculate the CRC of the data excluding the last 4 bytes which contain the CRC itself
    uint32_t calculatedCrc = crc32(data, dataLength);
  
    // Log and return the result in one step
...
void Crc_testSampletest(void) {
    ...
    0x00, 0x20, 0x00, 0x80, 0xBB, 0xB9, 0x0B, 0x78, 0xEE, 0x42, 0x10, 0x00, 0x01, 0x00, 0x62, 0x32, 
    0x01,0x38, 0x4A, 0x53, 0x20, 0xC5, 0x88, 0x20, 0x5F, 0xE7, 0x3B, 0x01, 0x60, 0x02, 0x00, 0x80, 
    0x7D, 0x76,0x88, 0x7A, 0x0D, 0x7C, 0x54, 0x08, 0x6C, 0x07, 0x13, 0x52, 0x08, 0xF4, 0x31, 0x87, 
    0x40, 0x1F, 0xA0,0x7F, 0x09, 0xC2, 0x1B, 0x4B, 0xA0, 0x25, 0xA0, 0x40, 0xA7, 0x3C, 0xDB, 0xED, 
    0xE2, 0xF0, 0xC8
};
bool status =0;
    uint32_t crctests = crc32(data, (sizeof(data)-4));
    printf("CRC32: 0x%08x\n", crctests);

    // test case for Verify CRc function

    status = verifyCrc32(data, sizeof(data));
    if(status == true)
    {
        printf("Test Passed!\r\n");
    }
